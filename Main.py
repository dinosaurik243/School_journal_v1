...
from PyQt5.QtWidgets import QMainWindow, QApplication, QWidget, QTableWidgetItem, QMessageBox, QCalendarWidget, \
    QScrollBar
from Designs import Pupil_form, Main_window, add_subject_form
import sqlite3
...
# Функция для определения следующего учебного года(не дает отображать даты, которые не входят в УЧЕБНЫЙ год)
...
# проверяет, превышает ли разница между ними 366 дней(год)
...
# Эта функция возвращает список дат между двумя заданными датами (включительно),
# за исключением выходных дней (субботы и воскресенья) и месяцев летних каникул (июня, июля, августа).
...
class MainWindow(QMainWindow, Main_window.Ui_MainWindow):
    ...
    # Метод показа таблицы(ТУТ ВЫГРУЗКА ИЗ БАЗ ДАННЫХ)
    def show_pupils(self):
        ...
        if find_db() == 11:
            ...
            self.error_lab.setText('К сожалению, базы данных отсутствуют')
            self.error_lab.show()

    # Метод, который загружает в QTableWidget данные
